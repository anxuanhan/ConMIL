...
try:
    ...
    HAS_OPENSLIDE = False
    print("⚠️  OpenSlide is not installed. Falling back to tifffile as an alternative reader.")
    try:
        import tifffile
        HAS_TIFFFILE = True
    except ImportError:
...
def normalize_attention(attn, low_pct=1.0, high_pct=99.0, gamma=0.5):
    ...
    attn_gamma   = np.power(attn_norm, gamma)  
...
def draw_heatmap(
    ...
    print(f"📖 Reading WSI thumbnail (level={vis_level})...")
    ...
    if mask_path and os.path.exists(mask_path):
        ...
        print(f"   Tissue coverage in mask: {cov:.1f}%")
    else:
        print("   ⚠️ No mask provided. The whole slide will be displayed")


    attn_raw  = attn_weights.cpu().numpy().astype(np.float32)
    attn_norm = normalize_attention(attn_raw, low_pct, high_pct, gamma)

    print(f"   Normalized attention — min={attn_norm.min():.4f}  "
...
def save_top_patches(slide_path, coords, attn_weights, patch_size,
    ...
    if not HAS_OPENSLIDE:
        print("  ⚠️  OpenSlide is required to save top patches. Skipping.")
        return
...
def parse_args():
    ...
    p.add_argument("--slide",         required=True,  help="Path to WSI .tif file")
    p.add_argument("--mask",          default="",     help="Path to mask .tif file (leave empty to disable)")
    p.add_argument("--h5",            required=True,  help="Path to .h5 feature file")
    p.add_argument("--ckpt",          required=True,  help="Path to model checkpoint")
    p.add_argument("--text_feat_dir", required=True,  help="Directory containing pos/neg_features.npy")
    p.add_argument("--output",        default="./heatmap_v2.png")

    # Color normalization
    p.add_argument("--low_pct",   type=float, default=1.0,
                   help="Lower percentile for normalization (remove low-end noise)")

    p.add_argument("--high_pct",  type=float, default=99.0,
                   help="Upper percentile for normalization (avoid compression by outliers)")

    p.add_argument("--gamma",     type=float, default=0.5,
                   help="Gamma correction (<1 enhances low-attention regions, >1 compresses them)")
    # Top patches
