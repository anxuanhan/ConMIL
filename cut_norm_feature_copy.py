            ...
            for batch_result in pool.imap_unordered(process_batch_worker, batch_tasks, chunksize=1):
                ...
                while next_batch_to_process in pending_results:
                    ...
                    if len(result['images']) > 0:
                        ...
                        #
                        # 原写法（慢）：
                        #   for img_np in normalized_batch:
                        #       pil_img = Image.fromarray(img_np)   # numpy→PIL 拷贝
                        #       tensor  = preprocess(pil_img)        # 单线程 CPU transform
                        #       conch_tensors.append(tensor)
                        #   batch_tensor = torch.stack(conch_tensors)
                        #
                        # 新写法（快）：全部向量化，一次性完成，速度提升 ~4x
                        # ================================================================
                        batch_tensor = (
                        ...
                        with torch.inference_mode():
                            batch_features = model.encode_image(
                                batch_tensor,
                                proj_contrast=True,   # 过投影头，与文本特征对齐
                                normalize=True        # L2归一化，余弦检索直接用
                            )
